...
def create_graphql_router(session_maker: async_sessionmaker[AsyncSession]):
    async def get_context(request: Request):
        user = None
        async with session_maker() as db:  # ✅ Properly use async context
            ...
            # ✅ Return context that includes a *new* session maker for resolvers
            return {
                "db": db,
                "db_factory": session_maker,  # Optional if you spawn new ones in resolvers
                "request": request,
                "user": user,
            }
